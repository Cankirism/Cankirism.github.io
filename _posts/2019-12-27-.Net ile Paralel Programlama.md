 ...
 class Program
    {
        ...
        static void SeriHesapla(List<int> sayilar)
        {
            ...
            Console.WriteLine("Seri yöntem -  9 ile bölünen sayısı = {0}", bolunenler.ToString());
        }
        ...
        static void ParalelHesapla(List<int> sayilar)
        {
            ...
            Console.WriteLine("Paralel yöntem - 9 ile bölünen sayısı ={0}", prBolunenler.ToString());
        }
...
Yazımızın giriş cümlelerinde de bahsetmiştik.Bazen öyle durumlar olur ki, kalsik yöntem ile paralel yöntem çıktıları birbirinden farklı olabilir. İşte bu durum bunun en güzel örneklerinden. İki metodumuzda aynı değerler üzerinde hesaplama yaptılar ama farklı çıktı ürettiler.Daha hızlı çıktı alma hevesi, yanlış çıktı alma gibi sonucu doğurabilir.   
Örnek uygulamamızda Paralel sınıfının For methodu , birden fazla görev(task) oluşturup, bunları birden fazla thread'e çalışması için gönderdi. Bu thread'ler görevleri başlatarak, ortak değişken olan prBolunenler değişkenini artırdılar. Aslında problem tam bu noktada kaynaklanmaktadır. Her thread ortak değişkeni arttırdı ve hatalı sonuç üretilmesine sebebiyet verdi. 
