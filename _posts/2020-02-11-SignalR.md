...
Yukarıdaki örnekte, hubProxy tanımlayıp, SignalR Hub'ımıza bağlandık. Daha sonra sunucu tarafından gönderilen bildirim  "GetNotification" ile istemciye iletildi.    
...
```  
Yukarıdaki örnek, sunucuya mesaj gönderen istemcinin yani PushNotification methodunuz invoke eden istemcinin connectionID değeri alınarak,  Clients.Client(connectionId).GetNotification(message) kodu ile sadece o istemciye bildirim gönderilmesini sağladık.  
SignalR konusunda anlatacaklarımız şimdilik bu kadar.   
